...
class RecordFinancialTransactionPage extends StatefulWidget {
  ...
  @override
  _RecordFinancialTransactionPageState createState() =>
      _RecordFinancialTransactionPageState();
}

class _RecordFinancialTransactionPageState
    extends State<RecordFinancialTransactionPage> {
  ...
  Future<void> _fetchNextTransactionId() async {
    final url =
        'http://${localhostInstance.ipServer}/dashboard/myfolder/getCurrentTransactionId.php'; // Replace with your server URL
      ...
      if (responseBody['success']) {
        setState(() {
          _transactionIdController.text =
              responseBody['next_transaction_id'].toString();
        });
      } else {
        // Handle failure
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(
                  'Failed to fetch transaction ID: ${responseBody['message']}')),
        );
      }
    } else {
      // Handle server error
  ...
  Future<void> _recordTransaction() async {
    ...
    final url =
        'http://${localhostInstance.ipServer}/dashboard/myfolder/saveTransaction.php'; // Replace with your server URL
      ...
      if (response.statusCode == 200) {
        ...
        if (responseBody['success']) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
                content: Text(
                    'Failed to record transaction: ${responseBody['message']}')),
          );
        }
      } else {
        // Handle server error
